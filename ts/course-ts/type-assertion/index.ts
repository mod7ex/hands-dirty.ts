 ...
 */
export type IsTypeEqual<T1, T2> = IsNotAny<T1> extends false ? false : IsNotAny<T2> extends false ? false : [T1] extends [T2] ? ([T2] extends [T1] ? true : false) : false;

/**
 * Checks if T2 can be assigned to T1.
 */
export type IsTypeAssignable<T1, T2> = IsNotAny<T1> extends false ? false : IsNotAny<T2> extends false ? false : [T2] extends [T1] ? true : false;
